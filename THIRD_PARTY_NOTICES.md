...
Toche reuses and adapts components from the following projects. Each is cloned into the gitignored `vendor_reuse/` directory for reference only. Adapted code is transformed into Rust and committed under `src/` as Toche's own work.
...
| [RTK](https://github.com/rtk-ai/rtk) | Apache-2.0 | `5d32d07` | Reference — adapt filter & hook architecture | Command/tool-output reduction, Claude Code hook integration |
...
```
Full text: `vendor_reuse/rtk/LICENSE`
...
- **RTK**: Reference adaptation. Study filter pipeline architecture and Claude Code hook system. Toche implements its own deterministic reducer with RTK-informed patterns. The RTK binary is not bundled.
