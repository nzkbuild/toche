...
test("maps every published platform to its release archive", () => {
  assert.equal(releaseAsset("linux", "x64").archive, "toche-1.0.7-x86_64-unknown-linux-gnu.tar.gz");
  assert.equal(releaseAsset("win32", "x64").archive, "toche-1.0.7-x86_64-pc-windows-msvc.zip");
  assert.equal(releaseAsset("darwin", "x64").archive, "toche-1.0.7-x86_64-apple-darwin.tar.gz");
  assert.equal(releaseAsset("darwin", "arm64").archive, "toche-1.0.7-aarch64-apple-darwin.tar.gz");
});
